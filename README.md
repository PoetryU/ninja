...
Ninja 仅支持 qinglong v12.0+

[TG 频道](https://t.me/+p2XnqqmulzM0OGQ1)
...
2. 进容器内执行以下命令
   ...
   **进容器内执行以下命令**
   ...
   ```bash
   git clone https://github.com/PoetryU/ninja.git /ql/data/ninja
...
```bash
git clone https://github.com/PoertyU/ninja.git
cd ninja/backend
pnpm install
...
自定义常见活动：修改 `/ql/data/ninja/backend/static/activity.json` 即可

## 注意事项

- 重启后务必执行一次 `ql extra` 保证 Ninja 配置成功。

- 更新 Ninja 只需要在**容器**中 `data/ninja/backend` 目录执行 `git pull` 然后 `pm2 start`
